...
public class SortPlaneByMaxSpeedTest {
    ...
    @Test
    public void testSortPlaneByMaxSpeed() {
        List<String> actual = new ArrayList<>();
        List<String> expected = Arrays.asList("B-1B Lancer", "Boeing-737", "Bell X-14");
        planeByParametersSorter.sortPlaneByMaxSpeed(planeList).forEach(plane -> actual.add(plane.getPlaneModel()));
        Assert.assertEquals(actual, expected);
    }

    @Test
    public void testSortPlaneByMaxSpeedNegative() {
        List<String> actual = new ArrayList<>();
